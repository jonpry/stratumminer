...
bool hex2bin(unsigned char *p, const char *hexstr, size_t len)
{
	...
	while (*hexstr && len) {
		...
		if (sscanf(hex_byte, "%x", &v) != 1) {
			std::cout << "hex2bin sscanf '" << hex_byte <<
			    "' failed" << std::endl;
			return ret;
		}
 ...
 */
void
yPoolWorkMgr_parseHexStringLE(char *hexString, uint32 length, uint8 * output)
{
	uint32 lengthBytes = length / 2;
	for (uint32 i = 0; i < lengthBytes; i++) {
		// high digit
...
void
primecoinBlock_generateHeaderHash(primecoinBlock_t * primecoinBlock,
				  uint8 hashOutput[32])
{
...
void
primecoinBlock_generateBlockHash(primecoinBlock_t * primecoinBlock,
				 uint8 hashOutput[32])
{
	uint8 blockHashDataInput[512];
	memcpy(blockHashDataInput, primecoinBlock, 80);
	...
	bnPrimeChainMultiplier.SetHex(primecoinBlock->
				      mpzPrimeChainMultiplier.get_str(16));
	std::vector < unsigned char >bnSerializeData =
	    bnPrimeChainMultiplier.getvch();
 ...
 */
bool
jhMiner_pushShare_primecoin(uint8 data[256], primecoinBlock_t * primecoinBlock)
{
	if (workData.protocolMode == MINER_PROTOCOL_GETWORK) {
		// prepare buffer to send
		fStr_buffer4kb_t fStrBuffer_parameter;
		fStr_t *fStr_parameter =
		    fStr_alloc(&fStrBuffer_parameter, FSTR_FORMAT_UTF8);
		fStr_append(fStr_parameter, "[\"");	// \"]
		fStr_addHexString(fStr_parameter, data, 256);
		fStr_appendFormatted(fStr_parameter, "\",\"");
		fStr_addHexString(fStr_parameter,
				  (uint8 *) & primecoinBlock->serverData, 32);
		fStr_append(fStr_parameter, "\"]");
		// send request
		sint32 rpcErrorCode = 0;
		jsonObject_t *jsonReturnValue =
		    jsonClient_request(&jsonRequestTarget, "getwork",
				       fStr_parameter,
				       &rpcErrorCode);
		if (jsonReturnValue == NULL) {
			...
			jsonObject_t *jsonReturnValueBool =
			    jsonObject_getParameter(jsonReturnValue, "result");
			if (jsonObject_isTrue(jsonReturnValueBool)) {
				...
				printf
				    ("Server rejected share (BlockHeight: %d/%d nBits: 0x%08X)\n",
				     primecoinBlock->serverData.blockHeight,
				     jhMiner_getCurrentWorkBlockHeight
				     (primecoinBlock->threadIndex),
				     primecoinBlock->
				     serverData.client_shareBits);
				jsonObject_freeObject(jsonReturnValue);
				return false;
			}
		}
		...
		// printf("Queue share\n");
		xptShareToSubmit_t *xptShareToSubmit =
		    (xptShareToSubmit_t *) malloc(sizeof(xptShareToSubmit_t));
		memset(xptShareToSubmit, 0x00, sizeof(xptShareToSubmit_t));
		memcpy(xptShareToSubmit->merkleRoot, primecoinBlock->merkleRoot,
		       32);
		memcpy(xptShareToSubmit->prevBlockHash,
		       primecoinBlock->prevBlockHash, 32);
		...
		memcpy(xptShareToSubmit->chainMultiplier, &bnSerializeData[0],
		       lengthBN);
		...
		if (workData.xptClient && !workData.xptClient->disconnected)
			xptClient_foundShare(workData.xptClient,
					     xptShareToSubmit);
		else {
			std::cout <<
			    "Share submission failed. The client is not connected to the pool."
			    << std::endl;
		}
	} else if (workData.protocolMode == MINER_PROTOCOL_STRATUM) {
		if (workData.stratumClient && !workData.stratumClient->disconnected)
			stratumClient_foundShare(workData.stratumClient,
					     primecoinBlock);
		else {
			std::cout <<
			    "Share submission failed. The client is not connected to the pool."
...
static bool IsXptClientConnected()
{
	...
	__try {
		...
		if (workData.xptClient == NULL
		    || workData.xptClient->disconnected)
			return false;
#ifdef _WIN32
	}
...
void jhMiner_queryWork_primecoin()
{
	...
	jsonObject_t *jsonReturnValue =
	    jsonClient_request(&jsonRequestTarget, "getwork", NULL,
			       &rpcErrorCode);
	uint32 time2 = getTimeMilliseconds() - time1;
	// printf("request time: %dms\n", time2);
	if (jsonReturnValue == NULL) {
		printf("Getwork() failed with %serror code %d\n",
		       (rpcErrorCode > 1000) ? "http " : "",
		       rpcErrorCode >
		       1000 ? rpcErrorCode - 1000 : rpcErrorCode);
		workData.workEntry[0].dataIsValid = false;
		return;
	} else {
		jsonObject_t *jsonResult =
		    jsonObject_getParameter(jsonReturnValue, "result");
		jsonObject_t *jsonResult_data =
		    jsonObject_getParameter(jsonResult, "data");
		//jsonObject_t* jsonResult_hash1 = jsonObject_getParameter(jsonResult, "hash1");
		jsonObject_t *jsonResult_target =
		    jsonObject_getParameter(jsonResult, "target");
		jsonObject_t *jsonResult_serverData =
		    jsonObject_getParameter(jsonResult, "serverData");
		...
		uint8 *stringData_data =
		    jsonObject_getStringData(jsonResult_data,
					     &stringData_length);
...
#endif
		yPoolWorkMgr_parseHexString((char *)stringData_data,
					    std::min < unsigned long >(128 * 2,
								       stringData_length),
					    workData.workEntry[0].data);
		workData.workEntry[0].dataIsValid = true;
		// get server data
		uint32 stringServerData_length = 0;
		uint8 *stringServerData_data =
		    jsonObject_getStringData(jsonResult_serverData,
					     &stringServerData_length);
		memset(workData.workEntry[0].serverData, 0, 32);
		if (jsonResult_serverData)
			yPoolWorkMgr_parseHexString((char *)
						    stringServerData_data,
						    std::min(128 * 2, 32 * 2),
						    workData.
						    workEntry[0].serverData);
		// generate work hash
		uint32 workDataHash = 0x5B7C8AF4;
		for (uint32 i = 0; i < stringData_length / 2; i++) {
			workDataHash =
			    (workDataHash >> 29) | (workDataHash << 3);
...
{
	if (workData.protocolMode == MINER_PROTOCOL_GETWORK)
		return ((serverData_t *) workData.workEntry[0].
			serverData)->blockHeight;
	else if(workData.protocolMode == MINER_PROTOCOL_STRATUM)
		return blockHeight;
	else
		return ((serverData_t *) workData.workEntry[threadIndex].
			serverData)->blockHeight;
}
...
void *jhMiner_workerThread_getwork(void *arg)
{
	...
	while (true) {
		...
		uint8 serverData[32];
		while (workData.workEntry[0].dataIsValid == false)
			Sleep(200);
#ifdef _WIN32
		EnterCriticalSection(&workData.cs);
#else
		...
		for (uint32 i = 0; i < 128 / 4; i++) {
			*(uint32 *) (localBlockData + i * 4) =
			    _swapEndianessU32(*(uint32 *)
					      (localBlockData + i * 4));
		}
...
void *jhMiner_workerThread_stratum(void *arg)
{
	uint32_t threadIndex = static_cast < uint32_t > ((uintptr_t) arg);
	...
	while (true) {
		...
		primecoinBlock_t primecoinBlock = workData.pblock;
#ifdef _WIN32
		LeaveCriticalSection(&workData.cs);
#else
		pthread_mutex_unlock(&workData.cs);
...
void *jhMiner_workerThread_xpt(void *arg)
{
	...
	while (true) {
		...
		pthread_mutex_lock(&workData.cs);
#endif
		memcpy(localBlockData, workData.workEntry[threadIndex].data,
		       128);
		memcpy(serverData, workData.workEntry[threadIndex].serverData,
		       32);
...
void jhMiner_printHelp()
{
	...
	cout <<
	    "  -o, -O <url>                   The miner will connect to this url"
	    << endl;
	cout <<
	    "                                 You can specifiy an port after the url using -o url:port"
	    << endl;
	cout <<
	    "  -u <string>                    The username (workername) used for login"
	    << endl;
	cout << "  -p <string>                    The password used for login"
	    << endl;
	...
	cout <<
	    ...
	    << endl;
	cout <<
	    "  -s <num>                       Set MaxSieveSize range from 200000 - 10000000"
	    << endl;
	cout << "                                 Default is 1500000." << endl;
	cout <<
	    "  -d <num>                       Set SievePercentage - range from 1 - 100"
	    << endl;
	cout <<
	    "                                 Default is 15 and it's not recommended to use lower values than 8."
	    << endl;
	cout <<
	    "                                 It limits how many base primes are used to filter out candidate multipliers in the sieve."
	    << endl;
	cout <<
	    "  -r <num>                       Set RoundSievePercentage - range from 3 - 97"
	    << endl;
	cout <<
	    "                                 The parameter determines how much time is spent running the sieve."
	    << endl;
	cout <<
	    "                                 By default 80% of time is spent in the sieve and 20% is spent on checking the candidates produced by the sieve"
	    << endl;
	cout <<
	    "  -primes <num>                  Sets how many prime factors are used to filter the sieve"
	    << endl;
	cout <<
	    "                                 Default is MaxSieveSize. Valid range: 300 - 200000000"
	    << endl;
	cout << "Example usage:" << endl;
#ifndef _WIN32
	cout <<
	    "  jhprimeminer -o http://poolurl.com:3333 -u workername -p workerpass"
	    << endl;
#else
	cout <<
	    "  jhPrimeminer.exe -o http://poolurl.com:8332 -u workername.1 -p workerpass -t 4"
	    << endl;
...
void jhMiner_parseCommandline(int argc, char **argv)
{
	...
	while (cIdx < argc) {
		...
		cIdx++;
		if (memcmp(argument, "-o", 3) == 0
		    || memcmp(argument, "-O", 3) == 0) {
			// -o
			if (cIdx >= argc) {
			...
			if (strstr(argv[cIdx], "http://"))
				commandlineInput.host =
				    fStrDup(strstr(argv[cIdx], "http://") + 7);
			else
			...
			// -p
			if (cIdx >= argc) {
				cout << "Missing password after -p option" <<
				    endl;
				exit(0);
			}
			...
			// -t
			if (cIdx >= argc) {
				cout << "Missing thread number after -t option"
				    << endl;
				exit(0);
			}
			commandlineInput.numThreads = atoi(argv[cIdx]);
			if (commandlineInput.numThreads < 1
			...
			// -s
			if (cIdx >= argc) {
				cout << "Missing number after -s option" <<
				    endl;
				exit(0);
			}
			commandlineInput.sieveSize = atoi(argv[cIdx]);
			if (commandlineInput.sieveSize < 200000
			    || commandlineInput.sieveSize > 40000000) {
				printf
			...
			// -s
			if (cIdx >= argc) {
				cout << "Missing number after -d option" <<
				    endl;
				exit(0);
			}
			commandlineInput.sievePercentage = atoi(argv[cIdx]);
			if (commandlineInput.sievePercentage < 1
			    || commandlineInput.sievePercentage > 100) {
				cout <<
				    "-d parameter out of range, must be between 1 - 100"
				    << endl;
				exit(0);
			}
			cIdx++;
			...
			// -s
			if (cIdx >= argc) {
				cout << "Missing number after -r option" <<
				    endl;
				exit(0);
			}
			commandlineInput.roundSievePercentage =
			    atoi(argv[cIdx]);
			if (commandlineInput.roundSievePercentage < 3
			    || commandlineInput.roundSievePercentage > 97) {
				cout <<
				    "-r parameter out of range, must be between 3 - 97"
				    << endl;
				exit(0);
			}
			cIdx++;
			...
			// -primes
			if (cIdx >= argc) {
				cout << "Missing number after -primes option" <<
				    endl;
				exit(0);
			}
			commandlineInput.sievePrimeLimit = atoi(argv[cIdx]);
			if (commandlineInput.sievePrimeLimit < 300
			    || commandlineInput.sievePrimeLimit > 200000000) {
				cout <<
				    "-primes parameter out of range, must be between 300 - 200000000"
				    << endl;
				exit(0);
			}
			cIdx++;
			...
			// -c
			if (cIdx >= argc) {
				cout << "Missing number after -c option" <<
				    endl;
				exit(0);
			}
			commandlineInput.L1CacheElements = atoi(argv[cIdx]);
			if (commandlineInput.L1CacheElements < 300
			    || commandlineInput.L1CacheElements > 200000000
			    || commandlineInput.L1CacheElements % 32 != 0) {
				cout <<
				    "-c parameter out of range, must be between 64000 - 2000000 and multiply of 32"
				    << endl;
				exit(0);
			}
			cIdx++;
			...
			// -primes
			if (cIdx >= argc) {
				cout << "Missing number after -m option" <<
				    endl;
				exit(0);
			}
			commandlineInput.primorialMultiplier = atoi(argv[cIdx]);
			if (commandlineInput.primorialMultiplier < 5
			    || commandlineInput.primorialMultiplier > 1009) {
				cout <<
				    "-m parameter out of range, must be between 5 - 1009 and should be a prime number"
				    << endl;
				exit(0);
			}
			cIdx++;
			...
			// -tune
			if (cIdx >= argc) {
				cout << "Missing flag after -tune option" <<
				    endl;
				exit(0);
			}
			if (memcmp(argument, "true", 5) == 0
			    || memcmp(argument, "1", 2) == 0)
			...
			// -target
			if (cIdx >= argc) {
				cout << "Missing number after -target option" <<
				    endl;
				exit(0);
			}
			commandlineInput.targetOverride = atoi(argv[cIdx]);
			if (commandlineInput.targetOverride < 0
			    || commandlineInput.targetOverride > 100) {
				printf
				    ("-target parameter out of range, must be between 0 - 100");
				exit(0);
			}
			cIdx++;
			...
			if (cIdx >= argc) {
				printf
				    ("Missing number after -bttarget option\n");
				exit(0);
			}
			commandlineInput.targetBTOverride = atoi(argv[cIdx]);
			if (commandlineInput.targetBTOverride < 0
			    || commandlineInput.targetBTOverride > 100) {
				printf
				    ("-bttarget parameter out of range, must be between 0 - 100");
				exit(0);
			}
			cIdx++;
			...
			// -primorial
			if (cIdx >= argc) {
				cout << "Missing number after -primorial option"
				    << endl;
				exit(0);
			}
			commandlineInput.initialPrimorial = atoi(argv[cIdx]);
			if (commandlineInput.initialPrimorial < 11
			    || commandlineInput.initialPrimorial > 1000) {
				cout <<
				    "-primorial parameter out of range, must be between 11 - 1000"
				    << endl;
				exit(0);
			}
			cIdx++;
			...
			// -target
			if (cIdx >= argc) {
				cout << "Missing number after -se option" <<
				    endl;
				exit(0);
			}
			commandlineInput.sieveExtensions = atoi(argv[cIdx]);
			if (commandlineInput.sieveExtensions <= 1
			    || commandlineInput.sieveExtensions > 15) {
				cout <<
				    "-se parameter out of range, must be between 0 - 15"
				    << endl;
				exit(0);
			}
			cIdx++;
			...
			// -debug
			if (cIdx >= argc) {
				cout << "Missing flag after -debug option" <<
				    endl;
				exit(0);
			}
			if (memcmp(argument, "true", 5) == 0
		...
		else if (memcmp(argument, "-help", 6) == 0
			 || memcmp(argument, "--help", 7) == 0) {
			jhMiner_printHelp();
			exit(0);
		} else {
			cout << "'" << argument << "' is an unknown option." <<
			    endl;
#ifdef _WIN32
			cout << "Type jhPrimeminer.exe -help for more info" <<
			    endl;
#else
...
static void CacheAutoTuningWorkerThread(bool bEnabled)
{
	...
	while (true && bEnabled && !appQuitSignal) {
		...
		printf("Auto Tuning in progress: %u %%\n",
		       ((primeStats.nL1CacheElements -
			 nL1CacheElementsStart) * 100) / (nL1CacheElementsMax -
							  nL1CacheElementsStart));

		float ratio =
		    primeStats.nWaveTime ==
		    0 ? 0 : ((float)primeStats.nWaveTime /
			     (float)(primeStats.nWaveTime +
				     primeStats.nTestTime)) * 100.0;
		printf
		    ("WaveTime %u - Wave Round %u - L1CacheSize %u - TotalWaveTime: %u - TotalTestTime: %u - Ratio: %.01f / %.01f %%\n",
		     primeStats.nWaveRound ==
		     0 ? 0 : primeStats.nWaveTime / primeStats.nWaveRound,
		     primeStats.nWaveRound, nL1CacheElements,
		     primeStats.nWaveTime, primeStats.nTestTime, ratio,
		     100.0 - ratio);
	...
	__try {
		...
		while (true) {
			...
			if (bOptimalL1SearchInProgress
			    || !bEnablenPrimorialMultiplierTuning
			    || !IsXptClientConnected())
				continue;
			...
			if (ratio > nRoundSievePercentage + 5) {
				if (!PrimeTableGetNextPrime((unsigned int)
							    primeStats.nPrimorialMultiplier))
					error
					    ("PrimecoinMiner() : primorial increment overflow");
				printf
				...
				if (primeStats.nPrimorialMultiplier > 2) {
					if (!PrimeTableGetPreviousPrime
					    ((unsigned int)
					     primeStats.nPrimorialMultiplier))
						error
						    ("PrimecoinMiner() : primorial decrement overflow");
					printf
					    ("Sieve/Test ratio: %.01f / %.01f %%  - New PrimorialMultiplier: %u\n",
					     ratio, 100.0 - ratio,
					     primeStats.nPrimorialMultiplier);
...
void PrintCurrentSettings()
{
	...
	cout << "Number of mining threads (-t): " << commandlineInput.numThreads
	    << endl;
	cout << "Sieve Size (-s): " << nMaxSieveSize << endl;
	cout << "Sieve Percentage (-d): " << nSievePercentage << endl;
	cout << "Round Sieve Percentage (-r): " << nRoundSievePercentage <<
	    endl;
	cout << "Prime Limit (-primes): " << commandlineInput.sievePrimeLimit <<
	    endl;
	cout << "Primorial Multiplier (-m): " << primeStats.nPrimorialMultiplier
	    << endl;
	cout << "L1CacheElements (-c): " << primeStats.nL1CacheElements << endl;
	cout << "Chain Length Target (-target): " << nOverrideTargetValue <<
	    endl;
	cout << "BiTwin Length Target (-bttarget): " << nOverrideBTTargetValue
	    << endl;
	cout << "Sieve Extensions (-se): " << nSieveExtensions << endl;
	cout << "Total Runtime: " << days << " Days, " << hours << " Hours, " <<
	    minutes << " minutes, " << seconds << " seconds" << endl;
	cout << "Total Share Value submitted to the Pool: " <<
	    primeStats.fTotalSubmittedShareValue << endl;
	cout <<
	    "--------------------------------------------------------------------------------"
	    << endl;
}
...
void *input_thread(void *)
{
	...
	while (true) {
		...
		case 'H':
			...
			if (!PrimeTableGetPreviousPrime
			    ((unsigned int &)primeStats.nPrimorialMultiplier))
				error
				    ("PrimecoinMiner() : primorial decrement overflow");
			std::cout << "Primorial Multiplier: " <<
			    primeStats.nPrimorialMultiplier << std::endl;
			break;
		case 'y':
		case 'Y':
			// explicit cast to ref removes g++ warning but might be dumb, dunno
			if (!PrimeTableGetNextPrime
			    ((unsigned int &)primeStats.nPrimorialMultiplier))
				error
				    ("PrimecoinMiner() : primorial increment overflow");
			std::cout << "Primorial Multiplier: " <<
		...
		case 'P':
			bEnablenPrimorialMultiplierTuning =
			    !bEnablenPrimorialMultiplierTuning;
			std::cout << "Primorial Multiplier Auto Tuning was " <<
			    (bEnablenPrimorialMultiplierTuning ? "Enabled" :
			     "Disabled") << std::endl;
			break;
		case 's':
		case 'S':
		...
		case 'T':
			...
			std::cout << "Round Sieve Percentage: " <<
			    nRoundSievePercentage << "%" << std::endl;
			break;
		case 'g':
		case 'G':
...
int jhMiner_main_getworkMode()
{
	...
	CreateThread(NULL, 0,
		     (LPTHREAD_START_ROUTINE) CacheAutoTuningWorkerThread,
		     (LPVOID) commandlineInput.enableCacheTunning, 0, 0);
	CreateThread(NULL, 0,
		     (LPTHREAD_START_ROUTINE) RoundSieveAutoTuningWorkerThread,
		     NULL, 0, 0);
	CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) input_thread, NULL, 0,
		     0);
	...
	threadHearthBeat =
	    (DWORD *) malloc(commandlineInput.numThreads * sizeof(DWORD));
	// start threads
	for (sint32 threadIdx = 0; threadIdx < commandlineInput.numThreads;
	     threadIdx++) {
		HANDLE hThread = CreateThread(NULL, 0,
					      (LPTHREAD_START_ROUTINE)
					      jhMiner_workerThread_getwork,
					      (LPVOID) threadIdx, 0, 0);
	...
	pthread_create(&threads[commandlineInput.numThreads], NULL,
		       input_thread, NULL);
	...
	// start threads
	for (uint32 threadIdx = 0; threadIdx < commandlineInput.numThreads;
	     threadIdx++) {
		pthread_create(&threads[threadIdx], &threadAttr,
			       jhMiner_workerThread_getwork, (void *)threadIdx);
	}
	...
	while (true) {
			...
			if (workData.workEntry[0].dataIsValid) {
				if (primeStats.bestPrimeChainDifficultySinceLaunch < primeDifficulty)
					primeStats.bestPrimeChainDifficultySinceLaunch
					    = primeDifficulty;
				printf
				    ("primes/s: %d best difficulty: %f record: %f\n",
				     (sint32) primesPerSecond,
				     (float)primeDifficulty, (float)
				     primeStats.bestPrimeChainDifficultySinceLaunch);
			}
		}
		...
		while (true) {
			uint32 passedTime =
			    getTimeMilliseconds() - time_updateWork;
			if (passedTime >= 4000)
				break;
			Sleep(200);
		}
...
void process()
{
	if (workData.protocolMode == MINER_PROTOCOL_STRATUM) {
		stratumClient_process(workData.stratumClient);
		char *disconnectReason = 0;
		if (workData.stratumClient == NULL
		    || stratumClient_isDisconnected(workData.stratumClient,
						    &disconnectReason)) {
			std::cout <<
			    "stratum: Disconnected, auto reconnect in 30 seconds"
			    << std::endl;
			if (workData.stratumClient && disconnectReason)
				std::cout << "xpt: Disconnect reason: "
				    << disconnectReason << std::endl;
			blockHeight = 0;
			Sleep(30 * 1000);
			if (workData.stratumClient)
				stratumClient_free(workData.stratumClient);
			while (true) {
				workData.stratumClient =
				    stratumClient_connect(&jsonRequestTarget);
				if (workData.stratumClient)
					break;
			}
		}
		...
		if (workData.xptClient == NULL
		    || xptClient_isDisconnected(workData.xptClient,
						&disconnectReason)) {
...
void stratumUpdate(){
	if(workData.stratumClient->blockHeight != blockHeight){
//		printf("New work\n");

		//TODO: lock client --client is sync

		uint32_t merkleSeed = rdtsc(); //rand() or something
		uint8_t txHash[32];
		uint8_t merkleRoot[32];

		bitclient_generateTxHash(workData.stratumClient->nonce1, sizeof(uint32), (uint8*)&merkleSeed, workData.stratumClient->coinb1_size, workData.stratumClient->coinB1, workData.stratumClient->coinb2_size, workData.stratumClient->coinB2, workData.stratumClient->merkles);
		bitclient_calculateMerkleRoot(workData.stratumClient->merkles, workData.stratumClient->nMerkle+1, merkleRoot);

		//Update the primecoin block
		pthread_mutex_lock(&workData.cs);
		workData.pblock.version = workData.stratumClient->version;
		memcpy(workData.pblock.prevBlockHash,workData.stratumClient->prevHash,32);
		memcpy(workData.pblock.merkleRoot,merkleRoot,32);
		workData.pblock.timestamp = workData.stratumClient->ntime;
		workData.pblock.nBits = workData.stratumClient->nbits;
		workData.pblock.extraNonce = merkleSeed;
		workData.pblock.serverData.blockHeight = workData.stratumClient->blockHeight;
		workData.pblock.serverData.nBitsForShare = workData.stratumClient->difficulty;

		//Notify workers to get new block
		blockHeight = workData.stratumClient->blockHeight;
		pthread_mutex_unlock(&workData.cs);

		//TODO: Unlock client --client is sync
...
//                                      double totalRunTime = (double)(getTimeMilliseconds() - primeStats.startTime);  unused?
			...
			double poolDiff =
			    GetPrimeDifficulty
			    (workData.stratumClient->difficulty);
			double blockDiff =
			    GetPrimeDifficulty(workData.stratumClient->
					       difficulty);
			std::
			    cout << std::endl <<
			    "--------------------------------------------------------------------------------"
			    << std::endl;
			std::cout << "New Block: " << blockHeight << " - Diff: " << blockDiff <<
			    " / " << poolDiff << std::endl;
			std::cout << "Valid/Total shares: [ " << valid_shares <<
			    " / " << total_shares << " ]  -  Max diff: " <<
			    primeStats.bestPrimeChainDifficultySinceLaunch <<
			    std::endl;
			statsPassedTime =
			    (double)(getTimeMilliseconds() -
				     primeStats.blockStartTime);

			if (statsPassedTime < 1.0)
				statsPassedTime = 1.0;	// avoid division by zero
			for (int i = 6; i <= std::max(6, (int)
						      primeStats.
						      bestPrimeChainDifficultySinceLaunch);
			     i++) {
				double sharePerHour = ((double)
						       primeStats.
						       chainCounter[0]
						       [i] / statsPassedTime)
				    * 3600000.0;
				std::cout << i << "ch/h: " <<
				    sharePerHour << " - " <<
				    primeStats.chainCounter[0]
				    [i] << " [ " << primeStats.chainCounter[1]
				    [i] << " / " << primeStats.chainCounter[2]
				    [i] << " / " << primeStats.chainCounter[3]
				    [i] << " ]" << std::endl;
			}
			std::cout <<
			    "Share Value submitted - Last Block/Total: "
			    << primeStats.fBlockShareValue <<
			    " / " <<
			    primeStats.fTotalSubmittedShareValue << std::endl;
			std::cout << "Current Primorial Value: "
			    << primeStats.nPrimorialMultiplier << std::endl;
			std::cout <<
			    "--------------------------------------------------------------------------------"
			    << std::endl;
...
void xptUpdate()
{
	...
	if (workData.xptClient
		...
		for (uint32 i = 0; i < workData.xptClient->payloadNum; i++) {
			...
			*(uint32 *) (blockData + 0) =
			    workData.xptClient->blockWorkInfo.version;
			memcpy(blockData + 4,
			       workData.xptClient->blockWorkInfo.prevBlock, 32);
			memcpy(blockData + 36,
			       workData.xptClient->workData[i].merkleRoot, 32);
			*(uint32 *) (blockData + 68) =
			    workData.xptClient->blockWorkInfo.nTime;
			*(uint32 *) (blockData + 72) =
			    workData.xptClient->blockWorkInfo.nBits;
			*(uint32 *) (blockData + 76) = 0;	// nonce
			memcpy(workData.workEntry[i].data, blockData, 80);
			((serverData_t *) workData.workEntry[i].serverData)->
			    blockHeight =
			    workData.xptClient->blockWorkInfo.height;
			((serverData_t *) workData.workEntry[i].
			 serverData)->nBitsForShare =
workData.xptClient->blockWorkInfo.nBitsShare;
...
//                                      double totalRunTime = (double)(getTimeMilliseconds() - primeStats.startTime);  unused?
			...
			double poolDiff =
			    ...
			    (workData.xptClient->blockWorkInfo.nBitsShare);
			double blockDiff =
			    GetPrimeDifficulty(workData.xptClient->
					       blockWorkInfo.nBits);
			std::
			    cout << std::endl <<
			    "--------------------------------------------------------------------------------"
			    << std::endl;
			std::cout << "New Block: " << workData.xptClient->
			    blockWorkInfo.height << " - Diff: " << blockDiff <<
			    " / " << poolDiff << std::endl;
			std::cout << "Valid/Total shares: [ " << valid_shares <<
			    " / " << total_shares << " ]  -  Max diff: " <<
			    primeStats.bestPrimeChainDifficultySinceLaunch <<
...
/*
 * Mainloop when using xpt/stratum mode
 */
int jhMiner_main_xptstratumMode()
{
#ifdef _WIN32
	// start the Auto Tuning thread
	CreateThread(NULL, 0,
	...
	std::map < DWORD, HANDLE > threadMap;
	threadHearthBeat =
	    (uint64 *) malloc(commandlineInput.numThreads * sizeof(uint64));
	// start threads
	for (sint32 threadIdx = 0; threadIdx < commandlineInput.numThreads;
	     threadIdx++) {
		HANDLE hThread = CreateThread(NULL, 0,
	...
	CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) watchdog_thread,
		     (LPVOID) & threadMap, 0, 0);
	...
	for (uint32 threadIdx = 0; threadIdx < commandlineInput.numThreads;
		...
		if (workData.protocolMode == MINER_PROTOCOL_STRATUM)
			pthread_create(&threads[threadIdx], &threadAttr,
			       jhMiner_workerThread_stratum, (void *)threadIdx);		
		else
			pthread_create(&threads[threadIdx], &threadAttr,
			       jhMiner_workerThread_xpt, (void *)threadIdx);
	}
	pthread_attr_destroy(&threadAttr);
#endif
	...
	while (true) {
		...
		if (loopCounter % 10 == 0) {
			...
			primeStats.primeLastUpdate = getTimeMilliseconds();
			primeStats.primeChainsFound = 0;
			float avgCandidatesPerRound =
			    (double)primeStats.nCandidateCount /
			    primeStats.nSieveRounds;
			float sievesPerSecond =
			    (double)primeStats.nSieveRounds / (statsPassedTime /
							       1000.0);
			primeStats.primeLastUpdate = getTimeMilliseconds();
			...
			uint32 bestDifficulty =
			    primeStats.bestPrimeChainDifficulty;
			primeStats.bestPrimeChainDifficulty = 0;
			float primeDifficulty =
			    GetChainDifficulty(bestDifficulty);
			if (workData.workEntry[0].dataIsValid || blockHeight > 0) {
				statsPassedTime =
				    (double)(getTimeMilliseconds() -
					     primeStats.blockStartTime);
				if (statsPassedTime < 1.0)
				...
				float shareValuePerHour =
				    primeStats.fShareValue / totalRunTime *
				    3600000.0;
				std::cout << "Val/h: " << shareValuePerHour <<
				    " - PPS: " << (sint32) primesPerSecond <<
				    " - SPS: " << sievesPerSecond << " - ACC: "
				    << (sint32) avgCandidatesPerRound <<
				    " - Primorial: " <<
				    primeStats.nPrimorialMultiplier << std::
				    endl;
				std::cout << " Chain/Hr:  ";
				for (int i = 6; i <= std::max(6, (int)
							      primeStats.bestPrimeChainDifficultySinceLaunch); i++) {
					std::
					    cout << i << ": " <<
					    std::setprecision(2) << (((double)
								      primeStats.chainCounter[0]
								      [i] /
								      statsPassedTime)
								     *
...
int main(int argc, char **argv)
{
	...
	if (commandlineInput.primorialMultiplier == 0) {
		...
		primeStats.nPrimorialMultiplier =
		    commandlineInput.primorialMultiplier;
	...
	std::cout <<
	    ...
	    << std::endl <<
	    " ============================================================================ "
	    << std::endl << "Launching miner..." << std::endl;
	...
	char ipText[32];
	esprintf(ipText, "%d.%d.%d.%d", ((ip >> 0) & 0xFF), ((ip >> 8) & 0xFF),
	...
	std::cout << "Connecting to '" << commandlineInput.host << "'" << std::
	    endl;

	// init stats
	primeStats.primeLastUpdate = primeStats.blockStartTime =
	...
	for (uint32 i = 0;
	     i < sizeof(primeStats.chainCounter[0]) / sizeof(uint32); i++) {
	...
	std::cout << "Using " << commandlineInput.numThreads << " threads" <<
	    std::endl;
	...
	if (commandlineInput.port == 10034) {
	...
	} else if (commandlineInput.port == 3333) {
		workData.protocolMode = MINER_PROTOCOL_STRATUM;
		std::cout << "Using stratum protocol" << std::endl;
		...
		printf("Warning: \n");
		printf
		    ("   GetWork() is outdated and inefficient. You are losing mining performance\n");
		printf
		    ("   by using it. If the pool supports it, consider switching to x.pushthrough.\n");
	...
	if (workData.protocolMode == MINER_PROTOCOL_GETWORK) {
		...
		workData.stratumClient = NULL;
		// stratum initial connect & login sequence
		while (true) {
			...
			while (true) {
				...
				if (workData.stratumClient != NULL)
					break;
				std::cout <<
				    "Failed to connect, retry in 30 seconds" <<
				    std::endl;
				Sleep(1000 * 30);
			}

			// make sure we are successfully authenticated
			while (stratumClient_isDisconnected
			       (workData.stratumClient, NULL) == false
			       &&
			       stratumClient_isAuthenticated
			       (workData.stratumClient)
			       == false) {
				stratumClient_process(workData.stratumClient);
				Sleep(1);
			...
			if (stratumClient_isDisconnected
			    (workData.stratumClient, &disconnectReason)
			    == true) {
				stratumClient_free(workData.stratumClient);
				workData.stratumClient = NULL;
				break;
			}
			if (stratumClient_isAuthenticated
			    (workData.stratumClient) == true) {
				std::
				    cout << "stratum login success" <<
				    std::endl;
				break;
			}
			if (disconnectReason) {
				std::cout << "stratum error: " <<
				    disconnectReason << std::endl;
			}
			// delete client
			stratumClient_free(workData.stratumClient);
			...
			std::cout <<
			    "stratum authentication sequence failed, retry in 30 seconds"
			    << std::endl;
			Sleep(30 * 1000);
		}
	} else if (workData.protocolMode == MINER_PROTOCOL_XPUSHTHROUGH) {
		...
		while (true) {
			...
			while (true) {
				workData.xptClient =
				    xptClient_connect(&jsonRequestTarget,
						      commandlineInput.numThreads);
				if (workData.xptClient != NULL)
					break;
				std::cout <<
			...
			// make sure we are successfully authenticated
			while (xptClient_isDisconnected
			       (workData.xptClient, NULL) == false
			       && xptClient_isAuthenticated(workData.xptClient)
			       == false) {
				xptClient_process(workData.xptClient);
				Sleep(1);
			}
			char *disconnectReason = NULL;
			// everything went alright?
			if (xptClient_isDisconnected
			    (workData.xptClient, &disconnectReason)
			    == true) {
				xptClient_free(workData.xptClient);
				workData.xptClient = NULL;
				break;
			}
			if (xptClient_isAuthenticated(workData.xptClient) ==
			    true) {
				break;
			}
			if (disconnectReason) {
				std::cout << "xpt error: " << disconnectReason
				    << std::endl;
			}
			// delete client
			xptClient_free(workData.xptClient);
			// try again in 30 seconds
			std::cout <<
			    "x.pushthrough authentication sequence failed, retry in 30 seconds"
			    << std::endl;
			Sleep(30 * 1000);
		}
	}
	...
	std::cout <<
	    ...
	    << std::endl <<
	    "===============================================================" <<
	    std::endl << "Keyboard shortcuts:" << std::endl <<
	...
	if (workData.protocolMode == MINER_PROTOCOL_GETWORK)
		return jhMiner_main_getworkMode();
		 ...
		 || workData.protocolMode == MINER_PROTOCOL_STRATUM)
		return jhMiner_main_xptstratumMode();
