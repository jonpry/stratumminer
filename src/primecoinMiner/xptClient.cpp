...
xptClient_t *xptClient_connect(jsonRequestTarget_t * target, uint32 payloadNum)
{
...
#ifdef _WIN32
	SOCKET clientSocket =
	    xptClient_openConnection(target->ip, target->port);
#else
	...
	unsigned int cbRet;
	WSAIoctl(clientSocket, FIONBIO, &nonblocking, sizeof(nonblocking), NULL,
		 0, (LPDWORD) & cbRet, NULL, NULL);
#else
	int flags, err;
	...
	xptClient->payloadNum =
	    std::max < uint32 > (1, std::min < uint32 > (127, payloadNum));
#ifdef _WIN32
...
void xptClient_sendWorkerLogin(xptClient_t * xptClient)
{
	...
	xptPacketbuffer_beginWritePacket(xptClient->sendBuffer,
					 XPT_OPC_C_AUTH_REQ);
	...
	// send to client
	send(xptClient->clientSocket,
	     (const char *)(xptClient->sendBuffer->buffer),
	     xptClient->sendBuffer->parserIndex, 0);
}

/*
 * Sends the share packet
 */
void xptClient_sendShare(xptClient_t * xptClient,
			 xptShareToSubmit_t * xptShareToSubmit)
{
	...
	// bnFixedMultiplier
	xptPacketbuffer_writeU8(xptClient->sendBuffer, &sendError,
				xptShareToSubmit->fixedMultiplierSize);
	xptPacketbuffer_writeData(xptClient->sendBuffer,
				  xptShareToSubmit->fixedMultiplier,
				  xptShareToSubmit->fixedMultiplierSize,
				  &sendError);
	// bnChainMultiplier
	xptPacketbuffer_writeU8(xptClient->sendBuffer, &sendError,
				xptShareToSubmit->chainMultiplierSize);
	xptPacketbuffer_writeData(xptClient->sendBuffer,
				  xptShareToSubmit->chainMultiplier,
				  xptShareToSubmit->chainMultiplierSize,
				  &sendError);
	// finalize
	xptPacketbuffer_finalizeWritePacket(xptClient->sendBuffer);
...
bool xptClient_process(xptClient_t * xptClient)
{
	...
	if (xptClient->list_shareSubmitQueue->objectCount > 0) {
		for (uint32 i = 0;
		     i < xptClient->list_shareSubmitQueue->objectCount; i++) {
			xptShareToSubmit_t *xptShareToSubmit =
			    (xptShareToSubmit_t *) xptClient->
			    list_shareSubmitQueue->objects[i];
	...
	sint32 bytesToReceive =
	    (sint32) (packetFullSize - xptClient->recvIndex);
	// packet buffer is always large enough at this point
	sint32 r =
	    recv(xptClient->clientSocket,
		 (char *)(xptClient->recvBuffer->buffer + xptClient->recvIndex),
		 bytesToReceive, 0);
	if (r <= 0) {
#ifdef _WIN32
	...
	if (xptClient->recvIndex == packetFullSize && packetFullSize == 4) {
		...
		if ((xptClient->recvSize + 4) >
		    xptClient->recvBuffer->bufferLimit) {
			xptPacketbuffer_changeSizeLimit(xptClient->recvBuffer,
							(xptClient->recvSize +
							 4));
		}
	}
...
void xptClient_foundShare(xptClient_t * xptClient,
			  xptShareToSubmit_t * xptShareToSubmit)
{
#ifdef _WIN32
	EnterCriticalSection(&xptClient->cs_shareSubmit);
